    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-xl-4 wow fadeInUp" data-wow-delay=".2s" style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInUp;">
                ...
                <div class="projectInfo">
                    <p><span><i class="lni lni-calendar"></i> Завършен на :</span> {{$pfpost->completed_at}}</p>
                    <p><span><i class="lni lni-code"></i> Skills:</span> {{$pfpost->skills}}</p>
                    <p><span><i class="lni lni-link"></i> Клиент:</span> <a href="{{$pfpost->client_url}}" target="_blank">{{$pfpost->client}}</a></p>
                </div>
                <p></p>
            </div>
        ...
        <div class="pagination">
            <ul class="d-flex justify-content-center align-items-center">
                @if($prev)
                    <li><a href="{{ route('pfpost.single', ['slug' => $prev->slug ]) }}"><i class="lni lni-arrow-left"></i> {{ $prev->title }}</a></li>
                @endif
                @if($next)
                    <li><a href="{{ route('pfpost.single', ['slug' => $next->slug ]) }}">{{ $next->title }} <i class="lni lni-arrow-right"></i></a></li>
                @endif
            </ul>
        </div>
